...
object IncomeList {
  ...
  case class Props(xs: List[Income]) {
    val incomeGroups: List[IncomeGroup] = xs
      .groupBy(i => i.since)
    ...
    val incomeGroupsWithSum: List[(IncomeGroup, BigDecimal)] =
      if (incomeGroups.isEmpty) List.empty
      else if (incomeGroups.size == 1) List(incomeGroups.head -> incomeGroups.head.amount)
      else
        val sums = incomeGroups.tail.scanLeft(incomeGroups.head.amount) { (acc, income) =>
          acc + income.amount
        }
    ...
    <.div(
      ...
      <.div(
        ...
        <.div(
          ...
          <.div(
            ^.className := "column text-align-right",
            <.span(formatting.formatCurrency(currentTotal)),
            <.br(),
            <.small(
              ^.className := "has-text-success",
              s"+${formatting.formatNumber(incomeGroup.percent(currentTotal))}%"
            )
          )
        )
      ),
  ...
  private def renderFn(props: Props): VdomNode = {
    <.div(
      props.incomeGroupsWithSum.zipWithIndex.map { case ((group, total), idx) =>
        renderIncomeGroup(group, total, idx)
      }.toVdomArray
    )
  }
