...
object FormExample {
  ...
  private def field(name:String, value:String)(validate:String => FormExample.Validated,
                                               onChange: String => SyncIO[Unit]): VdomNode =
    val validated = validate(value)

    <.div(^.className:="field",
      <.label(^.className:="label", name),
      <.div(^.className:="control",
        <.input.text(^.className:=s"input ${validated.cssClass}",
          ^.value := value,
  ...
  private def renderFn(state: Hooks.UseState[State]): VdomNode = {
    <.div(^.className:="mt-4",
      ...
      <.div(^.className := "columns",
        ...
        <.button(^.className:="column button is-primary",
