...
object MemoryCard {
  ...
  case class Props(cardDto: CardDto, onClick: String => IO[Unit])

  private def openCard(props: Props) =
    <.div(
      ^.className := "box memory-card memory-card--open",
      Icon.component(props.cardDto.iconProps),
      ^.onClick --> props.onClick(props.cardDto.id)
    )

  private def closedCard(props: Props) =
    <.div(^.className := "box memory-card memory-card--closed", ^.onClick --> props.onClick(props.cardDto.id))

  private def removedCard(props: Props) =
    <.div(^.className := "memory-card memory-card--removed", ^.onClick --> props.onClick(props.cardDto.id))
